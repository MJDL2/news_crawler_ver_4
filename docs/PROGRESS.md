...
#### 11. CLI 시스템 버그 발견 (v4.2.1 - 2025-05-29)
- [❌] **CLI help 시스템 무한 대기 버그 발견** (긴급 수정 필요)
  - `python main.py --help` 실행 시 응답 없이 무한 대기
  - `python main.py -q "검색어" --help` 동일한 현상
  - 원인: src/ui/cli.py의 인자 파싱 과정에서 대화형 모드로 진입
  - 임시 해결책: `run_interactive_v2.bat` 사용 권장
- [❌] **문서 구조 정합성 문제 발견**
  - README.md에 언급된 일부 디렉토리가 실제로 존재하지 않음
  - `dev/`, `docs/issues/`, `src/utils/monitoring.py` 등
  - 실제 구조와 문서 간 불일치 해결 필요
...
### 🔄 현재 상태: URL 수집 제한 로직 버그 수정 완료 (v4.2.2)
...
#### 긴급 수정 사항 (1주일 내)
1. **CLI 시스템 버그 수정** (최우선)
   - src/ui/cli.py의 help 옵션 처리 로직 수정
   - 인자 파싱 시 대화형 모드 진입 방지
   - argparse 시스템 전반적 점검

2. **문서 정합성 수정**
   - README.md 프로젝트 구조 섹션 실제 구조로 수정
   - 존재하지 않는 디렉토리/파일 언급 제거
   - USER_GUIDE.md에 안전한 실행 방법 가이드 추가
...
*최종 업데이트: 2025-05-29 17:00 (CLI 버그 발견 및 문서 정합성 문제 추가)*